...
- 左侧条目表（名称 / 大小 / 修改日期 / 种类，四列完整显示，支持多选，表头与列内容像素级对齐、列宽可拖拽）+ 右侧内容预览的分栏布局
- 顶部单行工具栏：全部展开 / 折叠、增加 / 提取 / 全部解压 / 删除 / 清理 / 注释与搜索框右对齐；窗口变窄时动作按钮自动收为图标（悬停提示名称）
...
```bash
# 引擎测试套件（143 项断言：回环/加密/互操作/分卷/格式识别/密码本/默认命名/动作可用性/排除规则等）
./Scripts/test.sh
